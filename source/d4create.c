...
#include "d4all.h"
   ...
   DATA4 *S4FUNCTION d4create( CODE4 *c4, const char *name, const FIELD4INFO *fieldData, const TAG4INFO *tagInfo )
   {
      ...
      /* !S4OFF_WRITE */
      static void dfile4createHeaderImageSet( FIELD4IMAGE *image, unsigned char len, unsigned char dec, char nullBinaryMask )
      {
         image->len = len ;
         image->dec = dec ;
         #ifdef S4FOX
                                     ...
                                     Bool5 hasCompressedData, Bool5 hasLongFieldNames ) //, short compressedBlockSize )
      {
         ...
         *hasMemo = 0 ;   // initialize to not having memo, if a memo field is found, change this at that time
         ...
         for ( int i = 0; i < (int)numFields; i++ )
         {
            ...
            {
               case r4str:
                  dfile4createHeaderImageSet( &createFieldImage, (unsigned char)(fieldData[i].len & 0xFF), (unsigned char)(fieldData[i].len >> 8),0 ) ;
                  break ;
               case r4date:
                  dfile4createHeaderImageSet( &createFieldImage, 8, 0, 0 ) ;
                  break ;
               case r4log:
                  dfile4createHeaderImageSet( &createFieldImage, 1, 0, 0 ) ;
                  break ;
               case r4num:
               case r4float:
                  dfile4createHeaderImageSet( &createFieldImage, (unsigned char)fieldData[i].len, (unsigned char)fieldData[i].dec, 0 ) ;
                  #ifdef E4MISC
                     {
     ...
     tempData->autoIncrementField = 0 ;
      ...
      if ( from->dataFile->openMdx == 1 )
      {
         ...
         c4->createTemp = 1 ;
            ...
            enum Collate4name oldCollation = c4->collateName ;
...
#endif /* S4FOX, !S4OFF_WRITE, S4COMPRESS */
...
{
   #if !defined( __WIN32 ) && !defined( __unix__ )  // LY Sep 2/04
      #ifndef S4MACINTOSH  // LY Jul 19/04
         assert5port( "function for d4compress" ) ;
      #endif
   #endif
