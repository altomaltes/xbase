...
void e4applyAscend( int keyType, char *buffer, unsigned long len )
{
   ...
   switch( keyType )
   {
      ...
      case r4log:
         ...
         case r5i2:
            {
               S4LONG i2asInt = len ;  /* LY 2001/07/28 : changed from long for 64-bit */
         ...
         case r4int:
            {
               S4LONG temp = len ;  /* LY 2001/07/28 : changed from long for 64-bit */
         ...
         case r4dateTimeMilli:  // AS Mar 10/03 - ms support in datetime
            t4dateTimeToFoxMilli( buffer, ((S4LONG *)buffer) ) ;
            break ;
         case r4dateTime:
            /* LY 2001/07/28 : changed from long* for 64-bit */
            t4dateTimeToFox( buffer, ((S4LONG *)buffer) ) ;
            break ;
      #endif /* S4FOX */
...
long S4FUNCTION time4long( const char *time, int strLen, Bool5 isMilli )
{
   ...
   if ( isMilli == 1 && time[8] != 0 && strLen > 8 )
   {
    ...
    , MUST4LOAD }
      ...
      */
      ,{
         ...
         &unused4filler,                                   // void *charToKeyCompressionArray
         &unused4filler,                                   // void *unicodeToKeyCompressionArray
         0,                                                // unsigned short keySizeCharPerCharAdd
         UNUSED4,                                          // unsigned short expandOrCompressChar
         UNUSED4,                                          // unsigned short noTailChar
         0,                                                // Bool5 didAllocChar
         0,                                                // Bool5 didAllocUnicode
      ...
      */
      ...
      ,{
         ...
         MUST4GENERATE_ARRAY,                              // void *unicodeToKeyTranslationArray
         (void *)cp850generalCompressArray,                // void *charToKeyCompressionArray
         MUST4GENERATE_ARRAY,                              // void *unicodeToKeyCompressionArray
         NEED4ONE_EXTRA_BYTE,                              // unsigned short keySizeCharPerCharAdd
         EXPAND4CHAR_TO_TWO_BYTES,                         // unsigned short expandOrCompressChar
         NO4TAIL_BYTES,                                    // unsigned short noTailChar
...
void t4unicodeToMachine( COLLATE4 *collate, char *output, const char *input, const int numBytes, int *lenOut )
{
   ...
   void t4strToDateTime( COLLATE4 *collate, char *result, const char *input, const int len, int *lenOut )
   {
      ...
      if ( len < 16 )
      {
         ...
         dt[1] = x4reverseLong( &tmpLong ) ;
      #else
         dt[0] = date4long( input ) ;
         dt[1] = time4long( input + 8, len - 8, 0 ) ;
      #endif
      t4dateTimeToFox( result, dt ) ;

      *lenOut = 2 * sizeof( S4LONG ) ;  /* LY 2001/07/28 : changed from long for 64-bit */
   }


   void t4strToDateTimeMilli( COLLATE4 *collate, char *result, const char *input, const int len, int *lenOut )
   {
      // AS Mar 10/03 - milli second included for type r4dateTimeMilli, look for appended ".MMM"
      // AS 06/26/00 - not requite that collate be 0
      // assert5( collate == 0 && lenOut != 0 ) ;  // should not be used.
      ...
      #else
         dt[0] = date4long( input ) ;
         dt[1] = time4long( input + 8, len - 8, 1 ) ;
      #endif
      t4dateTimeToFoxMilli( result, dt ) ;
   ...
   void t4strToInt( COLLATE4 *collate, char *result, const char *input, const int len, int *lenOut )
   {
      ...
      *lenOut = sizeof( S4LONG ) ;  /* LY 2001/07/28 */
   }
   ...
   // AS Jul 21/05 - Support for new field type binary float
      ...
      #pragma argsused
   #endif
   void t4strToFloat( COLLATE4 *collate, char *result, const char *input, const int len, int *lenOut )
   {
      assert5( lenOut != 0 ) ;  // should not be used.
      ...
      *lenOut = sizeof( float ) ;  /* LY 2001/07/28 */
   }
#endif
   ...
   void t4strToTime( COLLATE4 *collate, char *result, const char *input, const int len, int *lenOut )
   {
      ...
      // AS May 21/04 - this conversion was incorrect, it should be using the foxToDBTime conversion...
      ...
      time->hour = (short)c4atol( input, 2 ) ;
   ...
   void t4intToFox( char *result, const S4LONG *val )
   {
      int isPositive ;
         ...
         S4LONG tempLong ;  /* LY 2001/07/28 : changed from long to S4LONG for 64-bit */
         memcpy( &tempLong, (void*)val, sizeof(S4LONG) ) ; /* LY 00/11/09 : cast to void* for IA-64 */  /* LY 2001/07/28 */
         ...
         memcpy( result, &tempLong, sizeof(S4LONG) ) ;   /* LY 2002/12/17 : result can be on wrong byte boundary */
            ...
            isPositive = *val > 0 ;
         #endif
         *((S4LONG *)result) = x4reverseLong( val ) ;
      #endif
      if ( isPositive )
   ...
   void t4strToFox( COLLATE4 *collate, char *result, const char *inputPtr, const int inputPtrLen, int *lenOut )
   {
      ...
      //   t4dblToFox( result, x4reverseDouble( &tempDoub ) ) ;
      //#else
         // assert5( collate == 0 && lenOut != 0 ) ;  // should not be used.
...
void t4strToDbTimeStamp( COLLATE4 *collate, char *toPtr, const char *fromPtr, const int numLen, int *lenOut )
{
   ...
   if ( numLen > 17 )  // there is a fractional component...
      ...
      dbTimeStampResult.hour = x4reverseShort( &dbTimeStampResult.hour ) ;
      dbTimeStampResult.minute = x4reverseShort( &dbTimeStampResult.minute ) ;
      dbTimeStampResult.second = x4reverseShort( &dbTimeStampResult.second ) ;
      dbTimeStampResult.fraction = x4reverseLong( &dbTimeStampResult.fraction ) ;
   #endif
   ...
   *lenOut = sizeof( DBTIMESTAMP ) ;
   ...
   void t4curToFox( char *result, const CURRENCY4 *source )
   {
      ...
      char buffer[8] ;
      ...
      #ifdef S4DATA_ALIGN  /* LY 00/07/24 : for S4WINCE */
         short int tempShrt ;
         // LY Jun 29/04 : commented out S4WIN64 and changed from (void*)(source+sizeof(short int)*3)
         // to (char *)source + sizeof(short int) * 3 to avoid compiler optimization issue
         // (assembly code treating source->lo[3] as short -> alignment exception)
         // #ifdef S4WIN64 /* LY 2001/08/24 : restored old WinCE code to avoid address arithmentic errors */
            ...
            memcpy( &tempShrt, (char *)source + sizeof(short int) * 3, sizeof(short int) ) ;
         // #else
         //    memcpy( &tempShrt, &source->lo[3], sizeof(short int) ) ;
         // #endif
...
// AS 05/26/99 only used for ole-db, avoid due to non-unix
   ...
   */
   ...
   /* LY 2001/07/28 : changed input from long for 64-bit */
   void t4dateTimeToFox( char *result, const S4LONG *input )
   {
      double date, time ;
      double val ;
      ...
      assert5( flags4initialized == 1 ) ;
      ...
      #ifdef S4WIN64  /* LY 2001/07/28 : changed from S4DATA_ALIGN */
         memcpy( &tempLong, (void*)input, sizeof(S4LONG) ) ;  /* LY 2001/02/21 */
         date = (double) tempLong ;
         memcpy( &tempLong, (void*)(input+1), sizeof(S4LONG) ) ;  // CS 2012/07/26 Fix value retrieve
         ...
         long tmLong = tempLong - extraTime ;
            ...
            /* LY July 14/03 : changed from S4WINCE to !WORDS_BIGENDIAN */
               ...
               date = (double) tempLong ;
               // LY Jun 29/04 : added (char *) to &input[1], to avoid compiler
               // optimization issue (assembly treating input[1] as long
               memcpy( &tempLong, (char *)&input[1], sizeof(S4LONG) ) ;
            #else
               date = (double) x4reverseLong( input ) ;
               tempLong = x4reverseLong( &input[1] ) ;
            #endif
            S4LONG extraTime = tempLong % 1000 ;  /* LY 2001/07/28 : changed from long */
            S4LONG tmLong = tempLong - extraTime ;  /* for 64-bit */
         #else
            date = (double)input[0] ;
            // AS 09/14/00 - FoxPro rounds up to the nearest second...
   ...
   // AS Mar 10/03 - ms support in datetime
   void t4dateTimeToFoxMilli( char *result, const S4LONG *input )
   {
      double date, time ;
      double val ;
      #if defined(S4DATA_ALIGN) || defined(WORDS_BIGENDIAN) || defined(S4WIN64)  // CS 2011/04/29
      ...
      assert5( flags4initialized == 1 ) ;
         ...
         long tmLong = tempLong ; // - extraTime ;
            ...
            /* LY 2002/10/08 : date not swapped under CE */
               ...
               memcpy( &tempLong, &input[0], sizeof(S4LONG) ) ;
               date = (double) tempLong ;
               memcpy( &tempLong, &input[1], sizeof(S4LONG) ) ;
            #else
               date = (double) x4reverseLong( input ) ;
               tempLong = x4reverseLong( &input[1] ) ;
            #endif
   ...
   void t4floatToFox( char *result, const float *flt )
   {
      ...
      isPositive = ( *flt >= 0 ) ;
         ...
         if ( isPositive )
         {
            memcpy( (char *)result, (char *)flt, 4 ) ;
            result[0] += (unsigned)0x80 ;
         }
         else /* negative */
         {
            for ( i = 0 ; i < 4 ; i++ )
               result[i] = (char) (~(*( (unsigned char *)flt + i ))) ;
         }
      #else
         // #ifdef S4BYTEORDER_2301
         //    memcpy( (char *)result, ((char *)&flt) + 4, 4 ) ;
         //    memcpy( ((char *)result) + 4, ((char *)&flt), 4 ) ;
         //    memcpy( (void *)&flt, result, sizeof(float) ) ;
         // #endif
         if ( isPositive )
         {
            for ( i = 0 ; i < 4 ; i++ )
               result[i] = *( (char *)flt + 3 - i ) ;
            result[0] += (unsigned)0x80 ;
         }
         else /* negative */
            for ( i = 0 ; i < 4 ; i++ )
               result[i] = (char) (~(*( (unsigned char *)flt + 3 - i ))) ;
      #endif
   }
