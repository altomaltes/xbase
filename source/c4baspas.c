...
#define r4check -5
   ...
   Bool5 S4FUNCTION code4compressedMemos( CODE4 *cb, Bool5 value )
   {
      ...
      #else
         temp = 0;
         error4describe(cb, e4notSupported, 0, "code4compressedMemos", 0, 0);
      #endif
   ...
   */
      ...
      int dec, int nulls )
   {
      ...
      {
         #ifdef S4WINCE
            memset( wideBuff, 0, 255*sizeof(unsigned short) ) ;
            ...
            c4utoa( wideBuff ) ;
         #endif
         buff = (char *) u4allocEr( cb, 256*sizeof(char) ) ;
         if ( buff )
         {
            #ifdef S4WINCE
               memcpy( buff, wideBuff, 255*sizeof(char) ) ;
            #else
               memcpy( buff, name, 255*sizeof(char) ) ;
            #endif
            c4upper( buff ) ;
         }
         else
         {
            error4describe( cb, e4memory, 00000L, "could not add field name", 0, 0 ) ;
            return 0 ;
         }
      }
