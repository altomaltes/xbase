   ...
   int memo4fileWriteBoundary( MEMO4FILE *f4memo, const char *ptr, const unsigned len, FILE4LONG pos )
   {
      ...
      if ( partialReadWrite )
      {
         ...
         else
         {
            ...
            // now get the difference from the end of the block
            posInset = (short)(file4longMod( pos, blockSize )) ;
            assert5( posInset <= blockSize ) ;

            // we need to consider that we want to write to file starting from posInset, and
            // writing to the end of the blockSize (this is because this is a partia write, and
            // in that case we are always writing
            // posInset = blockSize - posInset ;
         }
         unsigned long lenToCopy = len - posOffset ;

         // now we need to write out the last block...
         // first fill in the buffer.  Start filling in
         file4longAdd( &pos, posOffset ) ;
         char blockWrite[32] ;
         memcpy( blockWrite, ptr + posOffset, lenToCopy ) ;
         memset( blockWrite + lenToCopy, 0, (blockSize - lenToCopy) ) ;
...
#endif
{
   ...
   int rc ;
      ...
      // AS Oct 28/03 - support for mdx large files
      FILE4LONG compare2 ;
      file4longAssignLong( compare2, fileLen ) ;
      file4longAdd( &compare2, f4memo->blockSize*4 ) ;
      // AS Nov 20/03 - should be using compare2 not fileLen
      // if ( file4longGreaterLong( pos, fileLen ) )  // writing at least 4 blocks after end of file - must be corrupt
      if ( file4longGreaterLong( pos, compare2 ) )  // writing at least 4 blocks after end of file - must be corrupt
