/*r4save.c   (c)Copyright Sequiter Software Inc., 1988-1998.  All rights reserved  */
...
int S4FUNCTION report4save_report( REPORT4 *report, FILE4SEQ_WRITE *seq )
{
   ...
   sval = report->codeBase->calcList.nLink;
   if( save4short( seq, &sval ) < 0 )
      return -1;
...
int S4FUNCTION report4retrieve_object( FILE4SEQ_READ *seq, AREA4 *area )
{
   ...
   switch( obj_type_num )
   {
      ...
      case obj4type_total: /* TOTAL */
         calc = expr4calcLookup( seq->file->codeBase, (char *)data, strlen((char *)data) );
         if( !calc || !calc->total )
            goto ObjErr1;
   ...
   {
      obj->date_format = (char *)u4allocFree( area->report->codeBase, strlen(date_format)+1 );
...
int S4FUNCTION report4retrieve_calc2( FILE4SEQ_READ *seq, REPORT4 *report )
{
   ...
   if( is_total )
      ...
      total->resetExpression = (EXPR4 *)reset_expr_src;
      total->lookahead = lookahead;
      total->logcon = logcon;
      total->addCondition = (EXPR4 *)con_src;
      }
   else
      {
...
int S4FUNCTION report4retrieve_calc( FILE4SEQ_READ *seq, REPORT4 *report )
{
   ...
   while( calc )
   {
      ...
      {
         expr = expr4parse( report->relate->data, (char *)calc->total->resetExpression );
         u4free( calc->total->resetExpression );
         calc->total->resetExpression = expr;
      }
      if( calc->total && calc->total->addCondition )
      {
         expr = expr4parse( report->relate->data, (char *)calc->total->addCondition );
...
/*   EXPR4 *expr; */
   ...
   report->report_name = (char *)u4allocEr( report->codeBase, strlen(buffer)+1 );
   ...
   report->report_file_name = (char *)u4allocEr( report->codeBase, strlen(seq->file->name)+1 );
   ...
   report->report_caption = (char *)u4allocEr( report->codeBase, strlen(buffer)+1 );
   if( !report->report_caption )
   {
      report4free( report, 1, 1 );
      return NULL;
                                  ...
                                  char *buf, char *name_buf )
{
   ...
   report->report_name = (char *)u4allocEr( report->codeBase, 10 );
   if( !report->report_name )
      return -1;
