...
int code4ping(CODE4 *c4, long s, long r )
{
   ...
   if (receive)
   {
      ...
      connect4receive(connect, data, receive, code4timeout(c4) ) ;
      u4free(data) ;
   }
   return r4success ;
}
...
int connection4receiveMessage( CONNECTION4 *connection )
{
   ...
   for( ;; )
   {
      ...
      rc = connect4receive( connection->connect, &connection->packet.type, sizeof( connection->packet.type ), code4timeout( c4 ) ) ;
      if ( rc != r4success )
      {
         connection->connectionFailure = e4connection ;
         return error4( c4, rc < 0 ? rc : e4net, E90160 ) ;
      }
      messageType = ntohs(connection->packet.type) ;
      #ifdef S4COM_PRINT
         #ifdef S4SERVER
            sprintf(buffer, "Receiving Message: %s, on socket %d", s4connectionPrint( messageType ), connection->connect->connectBuffer.connectLow->sockr ) ;
      ...
      if ( messageType < 10000 )
      {
         ...
         connect4receive( connection->connect, &connection->packet.status,
            sizeof( connection->packet ) - sizeof( connection->packet.type ), code4timeout( c4 ) ) ;
         tempLen = connection4len(connection) ;
         ...
         if ( connection->bufferLen < tempLen )
         {
            ...
            tPtr = (char *)u4allocFree( c4, tempLen + 4 ) ;
            if ( tPtr == NULL )
               return e4memory ;
         ...
         if ( connect4receive( connection->connect, connection->buffer, tempLen, code4timeout( c4 ) ) != r4success )
         {
            connection->connectionFailure = e4connection ;
            return error4( c4, e4connection, E90160 ) ;
         }
         ...
         switch( messageType )
         {
            ...
            case STREAM4UNLOCK_DATA:
               ...
               connect4receive( connection->connect, &clientId, sizeof( clientId ), code4timeout( c4 ) ) ;
               connect4receive( connection->connect, &serverId, sizeof( serverId ), code4timeout( c4 ) ) ;
               clientId = ntohl(clientId) ;
               serverId = ntohl(serverId) ;
              /* if ( rc != r4success )*/
              /* {*/
              /*    connection->connectionFailure = e4connection ;*/
              /*    return error4( c4, e4connection, E90160 ) ;*/
            ...
            case STREAM4LOCKED_INFO:
               connect4receive( connection->connect, &c4->lockedLockItem, sizeof( long ), code4timeout( c4 ) ) ;
               c4->lockedLockItem = htonl( c4->lockedLockItem ) ;
               connect4receive( connection->connect, &len, sizeof( short ), code4timeout( c4 ) ) ;
               len = htons( len ) ;
               ...
               connect4receive( connection->connect, c4->lockedFileNameBuf, len, code4timeout( c4 ) ) ;
               ...
               connect4receive( connection->connect, c4->lockedUserNameBuf, len, code4timeout( c4 ) ) ;
               ...
               rc = connect4receive( connection->connect, c4->lockedNetNameBuf, len, code4timeout( c4 ) ) ;
      ...
      #else
         /* server will process message as well */

         #ifdef S4JAVA
            if ( messageType > JAVA4START_COUNT )  /* java message */
               rc = java4processMessage( c4, connection->connect->client, messageType ) ;
            else  /* must be a standard server message */
         #endif
            rc = server4processMessage( c4->server, connection->connect->client ) ;
      #endif
...
/*
   ...
   data may by NULL if we just want to reserve space. If data is NULL, atPtr
     cannot be NULL, and the reserved space is blanked out with NULLs.
...
int S4FUNCTION connection4receive( CONNECTION4 *connection, char *data, int len )
{
   ...
   rc = connect4receive( connection->connect, data, len, code4timeout( connection->cb ) ) ;
   if (rc == r4success )
      return len ;
   return rc ;
}
...
/*
   ...
   do
   {
      connection->packet.delayHundredthsIn = (htonl(delayHundredthsIn)) ;
      connection4sendMessage(connection) ;
      connection4clear(connection) ;
      rc = connection4receiveMessage(connection) ;
      if (rc < 0 )
         return rc ;
      rc = ntohs( connection->packet.status ) ;
      if ( rc != r4locked )
         return rc ;
      if ( numAttempts != -1 )
         numAttempts-- ;
