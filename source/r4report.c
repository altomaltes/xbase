...
#include "d4all.h"
   ...
   PREPORT4 S4FUNCTION report4init( RELATE4 *relate )
   {
      ...
      c4strcpy( r4->default_date_format, sizeof( r4->default_date_format ), code4dateFormat( codeBase ) );
      ...
      /* create the default style */
         ...
         if( !codeBase->s4cr2 )
         {
            ...
            lstrcpy(lfont.lfFaceName, TEXT("MS Serif"));
            #ifdef S4GERMAN
   ...
   HDC S4FUNCTION report4get_printerIC( REPORT4 *report )
   {
      ...
      LPTSTR szDevice, szDriver, szOutput ;
         ...
         {
            GetProfileString( TEXT("devices"), report->printer_name, TEXT("=,"), szPrinter, 80) ;
            szDevice = report->printer_name ;
            #ifdef UNICODE
               if ( ! (szDriver = wcstok( szPrinter, L", ")) ) return 0 ;
               if ( ! (szOutput = wcstok( NULL, L", ")) ) return 0 ;
            #else
               // AS Dec 13/05 - under Windows strtok is becoming deprecated...
               #ifdef S4WINDOWS_VS5_PLUS
                  char *context = 0 ;
                  if ( ! (szDriver = strtok_s( szPrinter, ", ", &context )) ) return 0 ;
                  if ( ! (szOutput = strtok_s( NULL, ", ", &context)) ) return 0 ;
               #else
         ...
         {
            GetProfileString( TEXT("windows"), TEXT("device"), TEXT(",,,"), szPrinter, 80 ) ;

            #ifdef UNICODE
               if ( ! (szDevice = wcstok( szPrinter, L",")) )  return 0 ;
               if ( ! (szDriver = wcstok( NULL, L", ")) ) return 0 ;
               if ( ! (szOutput = wcstok( NULL, L", ")) ) return 0 ;
            #else
               // AS Dec 13/05 - under Windows strtok is becoming deprecated...
               #ifdef S4WINDOWS_VS5_PLUS
                  char *context = 0 ;
                  if ( ! (szDevice = strtok_s( szPrinter, ",", &context )) )  return 0 ;
                  if ( ! (szDriver = strtok_s( NULL, ", ", &context)) ) return 0 ;
                  if ( ! (szOutput = strtok_s( NULL, ", ", &context)) ) return 0 ;
               #else
      ...
      //return CreateIC(szDriver, szDevice, szOutput, NULL) ;  // CS 2001/06/18
   ...
   void report4output_screen_fonts( REPORT4 *report )
   {
      ...
      while( style_on )
      {
         if( !style_on->printer_font )
         {
            ...
            //plf->lfHeight = -MulDiv(style_on->point_size, GetDeviceCaps(report->printerDC,LOGPIXELSY),72);
            plf->lfHeight = (long)(-1 * (LONGLONG)style_on->point_size * (LONGLONG)GetDeviceCaps(report->printerDC,LOGPIXELSY) / (LONGLONG)72);
   ...
   /* USED FOR CREATING A DIALOG TEMPLATE ON THE FLY, this is used to create
      ...
      LPDLGTEMPLATE PrintDlgTemplate( void )
      {
         ...
         lpDT = (LPDLGTEMPLATE)HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY, 1024 ) ;;
         if( !lpDT )
            return 0;
...
/* PreViewWin */
      if( GetClassInfo( reginst, TEXT("PreViewWin") , &temp ) )
         UnregisterClass( TEXT("PreViewWin"), reginst ) ;
      ...
      wc.hIcon = (HICON)NULL;
      wc.lpszClassName = TEXT("PreViewWin");
      #ifdef S4JAY
...
/* OutputPageWin */
      if( GetClassInfo( reginst, TEXT("OutputPageWin") , &temp ) )
         UnregisterClass( TEXT("OutputPageWin"), reginst ) ;

      wc.hbrBackground = (HBRUSH)GetStockObject( WHITE_BRUSH );
      wc.lpfnWndProc   = OutputPageProc;
      wc.lpszClassName = TEXT("OutputPageWin");
      #ifdef S4JAY
         if( (RegisterClass( &wc )) == NULL )
         {
...
/* MouseEat */
      if( GetClassInfo( reginst, TEXT("MouseEat") , &temp ) )
         UnregisterClass( TEXT("MouseEat"), reginst ) ;

      wc.lpszClassName = TEXT("MouseEat");
   ...
   ***************************************************************************/
   ...
   int S4FUNCTION report4caption( PREPORT4 report, char S4PTR *caption )
   {
      ...
      // AS Dec 13/05 vs 5.0 fixes
      int rclen = (strlen(caption)+1) * sizeof(TCHAR) ;
      report->report_caption = (LPTSTR)u4allocFree( report->codeBase, rclen );
      if( !report->report_caption )
      {
      ...
      #ifdef S4UNICODE
         c4atou(caption, report->report_caption, rclen );
      #else
         c4strcpy( report->report_caption, rclen, caption );
      #endif
   ...
   int S4FUNCTION report4hardResets( PREPORT4 report, int reset_flag )
   {
      ...
      {
         #ifdef S4WINCE   // CS 2001/07/04 no print setup dialog in CE
            return 0;
         #else
            static PRINTDLG pd;
   ...
   /* SEE THE CODEREPORTER MANUAL */
      ...
      int S4FUNCTION report4dataDo( PREPORT4 report )
      {
         ...
         for( i = 0; i < (int)report->output_objs.nLink; i++ )
         {
            ...
            // AS Dec 13/05 vs 5.0 fixes
            int fnlen = strlen(oobj->obj->field_name) + 1 ;
            f4infoi->name = (char *)u4allocFree( report->codeBase, fnlen );
            c4strcpy( f4infoi->name, fnlen, oobj->obj->field_name );
