/* f4open.c   (c)Copyright Sequiter Software Inc., 1988-1998.  All rights reserved. */
...
static int file4openLow( FILE4 *file, CODE4 *c4, const char *name )
{
   ...
   #endif  /* S4OFF_MULTI */
   #ifdef S4WINCE
      file->hand = CreateFile( nameU, fdwAccess, fdwShareMode, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0 ) ;
   #else
      file->hand = CreateFile( name, fdwAccess, fdwShareMode, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0 ) ;
   #endif
   if ( file->hand == INVALID4HANDLE )
      switch ( GetLastError( ) )
      {
