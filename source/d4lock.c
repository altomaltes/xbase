...
int S4FUNCTION d4lockInternal( DATA4 *data, const long rec, Bool5 doUnlock, Lock4type lockType )
{
   // default setting for lockType is LOCK4WRITE
      ...
      {
         #ifdef S4STAND_ALONE
            // AS Feb 12/03 - For ODBC Stand Alone, if anyone in our session holds the
            // lcok, we consider it locked, so return 0 in that case.
            if ( c4->odbc == 1 && rc == r4locked )
               return 0 ;
         #endif
         ...
         if ( rc == r4locked )
         {
            ...
            if ( doUnlock == 0 || code4unlockAuto( c4 ) != LOCK4ALL )  /* won't get unlocked in this case... */
            {
               ...
               if ( rc == 0 ) /* means nobody had it locked, must be a record lock */
                  if ( dfile4lockTest( data->dataFile, 0, data4serverId( data ), rec, lock4any ) == 0 )  /* not for our serverId, so will fail */
                  {
                     if ( c4->lockAttempts == WAIT4EVER )
                        return error4( c4, e4lock, E81523 ) ;
                     else
         ...
         if ( doUnlock )
         {
            switch( code4unlockAuto( c4 ) )
            {
               ...
               case LOCK4DATA :
                  ...
                  if ( rc == 0 )
                     rc = d4unlockAppendInternal( data, -1 ) ;
                  break ;
               default:
                  break ;
            }
         ...
         {
            // AS Apr 15/03 - support for new lockId for shared clone locking
            rc = dfile4lock( data->dataFile, data4lockId( data ), data4serverId( data ), rec ) ;
         }
         else
         {
            rc = 0 ;
   ...
   int S4FUNCTION d4lockAllInternal( DATA4 *data, Bool5 doUnlock )
   {
         ...
         if( rc )
         {
            switch( code4unlockAuto( data->codeBase ) )
            {
               ...
               case LOCK4DATA :
                  // AS Apr 15/03 - support for new lockId for shared clone locking
                  rc2 = d4unlockLow( data, data4lockId( data ), 0 ) ;
                  break ;
               default:
                  rc2 = 0 ;
                  break ;
...
int S4FUNCTION d4lockAppendInternal( DATA4 *data, Bool5 doUnlock )
{
      ...
      CODE4 *c4 ;
      int rc ;
      /* LY 2002/11/12 : added S4DATA_ALIGN */
      #if defined(S4BYTE_SWAP) || defined(S4DATA_ALIGN)  /* LY 2001/07/21 */
         double tempDbl ;
      #endif
      ...
      // AS Apr 15/03 - support for new lockId for shared clone locking
      rc = dfile4lockTestAppend( data->dataFile, data4lockId( data ), data4serverId( data ) ) ;
      if ( rc )  /* error or we have locked, or r4locked */
      {
         if ( rc == 1 )
            return 0 ;
         // AS Mar 11/03 - In the ODBC Server DLL, this is not an issue because
         // we use multi-threading and relax the lock contention internal to ODBC
         #ifndef S4ODBC_BUILD
            if ( rc == r4locked )
            {
               /* now, if another DATA4 has it locked, it may get unlocked below... */
            ...
            if ( rc == 0 )  // look to see if this is an auto-increment field...
            {
               ...
               if ( data->dataFile->file.lowAccessMode != OPEN4DENY_RW && data->autoIncrementField != 0 )
               {
                     ...
                     // in non-multi-user, need to refresh this value from disk...
                        ...
                        /* LY July 10/03 : restored tempDbl */
...
int S4FUNCTION d4lockFileInternal( DATA4 *data, Bool5 doUnlock, Lock4type lockType )
{
      ...
      {
         #ifdef S4STAND_ALONE
            // AS Feb 17/03 - For ODBC Stand Alone, if anyone in our session holds the
            // lcok, we consider it locked, so return 0 in that case.
            if ( c4->odbc == 1 && rc == r4locked )
               return 0 ;
         #endif
         ...
         if ( doUnlock )
         {
            ...
            if ( dfile4lockTestFileInternal( data->dataFile, data4lockId( data ), data4serverId( data ), lock4write ) != 0 )
               return error4( c4, e4result, E92709 ) ;
         #endif
         rc = dfile4lockFile( data->dataFile, data4lockId( data ), data4serverId( data ), lockType ) ;
         if ( lockType == lock4read )
         {
            ...
            for ( ;; )
            {
               ...
               #ifdef S4SERVER
                  // AS Apr 15/03 - support for new lockId for shared clone locking
                  if ( data4lockId( data ) == lock->clientId )
                  {
               #endif
                  #ifdef S4LOCK_HASH
            ...
            #ifdef S4SERVER
               lock->clientId = data4lockId( data ) ;
            #endif
            lock->data = data ;
            data->dataFile->fileReadLocks.add( (Single4lock *)lock ) ;
         }

         // AS May 1/03 - Was failing to set auot-increment val if entire table was locked
         #if defined( S4FOX ) && !defined( S4OFF_WRITE ) && !defined( S4UTILS ) && !defined( S4OFF_MULTI ) && !defined( S4UTILS )
            if ( rc == 0 )
            {
               if ( data->dataFile->file.lowAccessMode != OPEN4DENY_RW && data->autoIncrementField != 0 )
               {
                  #ifdef S4DATA_ALIGN  /* LY 2001/07/21 */
                     double tempDbl ;  // AS Mar 22/06 - ce fix
                     tempDbl = dfile4getAutoIncrementValue( data->dataFile ) ;
                     memcpy( data->dataFile->autoIncrementVal, (char*)&tempDbl, sizeof(double) ) ;
                  #else
                     data->dataFile->autoIncrementVal = dfile4getAutoIncrementValue( data->dataFile ) ;
                  #endif
                         ...
                         , Lock4type lockType )
{
      ...
      d4file = data->dataFile ;
         ...
         if ( rc )
            return rc ;
         rc = dfile4lockTestFileInternal( d4file, data4lockId( data ), data4serverId( data ), lock4read ) ;
         if ( lockType == lock4read )
         {
