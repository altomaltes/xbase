         ...
         if ( d4lockTestAppend( data ) != 1 )
         {
            ...
            char moreInfo[80] ;
               ...
               if ( data->dataFile->appendServerLock = 0 )
                  strcpy( moreInfo, "there are no locks on the append bytes" ) ;
               else
                  strcpy( moreInfo, "there are locks on the append bytes for a different client" ) ;
            #else
               strcpy( moreInfo, "stand alone version" ) ;
            #endif
