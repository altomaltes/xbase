...
int S4FUNCTION d4flush( DATA4 *data )
{
      ...
      return 0 ;
         ...
         // on the server if d4flush is called.
         ...
         if ( rc == 0 )
         {
            ...
            #ifdef E4ANALYZE
               if ( connection == 0 )
                  return error4( c4, e4parm, E92601 ) ;
            #endif
            connection4assign( connection, CON4FLUSH, data4clientId( data ), data4serverId( data ) ) ;
            connection4sendMessage( connection ) ;
            rc = connection4receiveMessage( connection ) ;
            if ( rc < 0 )
               return error4stack( c4, rc, E91717 ) ;
            if ( connection4type( connection ) != CON4FLUSH )
