/* r4drive2.c   (c)Copyright Sequiter Software Inc., 1988-1998.  All rights reserved. */
...
#endif
{
   ...
   switch (message)
   {
      case WM_INITDIALOG:
         ...
         SetWindowLong( hWnd, DWLP_USER, lParam );   // JACS
         report = (REPORT4 *)GetWindowLongPtr( hWnd, DWLP_USER );    // JACS
         if( !report )
            break;
      ...
      case WM_COMMAND:
         ...
         switch( cmd )
            ...
            case 111: /* next page */
               ...
               hCursor = SetCursor( LoadCursor( (HWND)NULL, IDC_WAIT ) ) ;
               ShowCursor( TRUE ) ;
...
int report4output_window( REPORT4 *report )
{
   ...
   hMenu = CreateMenu();
   #ifdef S4GERMAN
      AppendMenu( hMenu, MF_ENABLED | MF_STRING, 111, "&Nächste");
      AppendMenu( hMenu, MF_ENABLED | MF_STRING, 444, "&Schließen");
   #else
   ...
   #ifdef __WIN32
   SetClassLong( outputwindow, GCLP_HCURSOR, (LONG)((HCURSOR)LoadCursor((HWND)NULL, IDC_ARROW)) ); // JACS
   #else
   SetClassWord( (HWND)outputwindow, (int)GCW_HCURSOR, (WORD)(HCURSOR)LoadCursor((HWND)NULL, IDC_ARROW) );
   #endif
   ...
   pagewin = CreateWindow( "OutputPageWin","",WS_CHILD|WS_VISIBLE,
      xpos, 0, width, height,
      hWnd, NULL, (HINSTANCE)GetWindowLongPtr( hWnd, GWLP_HINSTANCE), (LPVOID)report );
   #else
   pagewin = CreateWindow( "OutputPageWin","",WS_CHILD|WS_VISIBLE,
      xpos, 0, width, height,
      hWnd, (HMENU)NULL, (HINSTANCE)GetWindowWord( hWnd, GWW_HINSTANCE), (void FAR *)report );
   #endif
   ReleaseDC( hWnd, hDC );
   ...
   #ifdef __WIN32
      SetClassLong( pagewin, GCLP_HCURSOR, (LONG)((HCURSOR)LoadCursor(NULL,IDC_ARROW)) ); // JACS
   #else
...
void report4evaluate_lookahead( GROUP4 *group )
{
   ...
   do
   {
      ...
      {
         #ifdef S4WINDOWS
         group_on->held_reset_value = (char *)u4alloc( lstrlen( group->lastResetValue ) + 1 );
         #else
...
int S4FUNCTION report4do( REPORT4 *report )
{
   ...
   else
   {
      ...
      #ifdef __WIN32
         hDlgPrint = CreateDialogIndirectParam( (HINSTANCE)GetWindowLongPtr( report->hWnd,GWLP_HINSTANCE )
                                              ,
                                                (LPCDLGTEMPLATE)dialogtemplate, report->hWnd,
