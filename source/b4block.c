...
double S4FUNCTION x4reverseDouble( const void *val )
{
   ...
   *to = *--from ;
   ...
   int b4dataLeaf( void *data, TAG4FILE *tag )
   {
      ...
      nKeys = * ((short *)data) ;
      // AS Oct 28/03 - verify valid if debug mode
   ...
   void b4leafInit( B4BLOCK *b4 )
   {
      ...
      // AS Jun 4/07 - the trailCntLen and dupCntLen need to be set first before the subtraction
      ...
      #ifdef E4INDEX_VERIFY
         assert5( !b4leaf( b4 ) || ( b4->curPos > (char *)(&b4->nodeHdr) + sizeof( b4->nodeHdr ) + b4->nodeHdr.freeSpace ) ) ;
      #endif
   }
   ...
   /* S4FOX */
      ...
      int S4FUNCTION x4dupCnt( const B4BLOCK *b4, const int numInBlock )
      {
         ...
         else
         {
            ...
            #endif
            // AS Jun 8/04 - for large key support, using a different trail byte counter
            return (int)( ( longPtr >> pos ) & b4->dupByteCnt ) ;
         #else
            #ifdef S4DO_BYTEORDER
               unsigned long longVal = *lPtr ;
               longVal = x4reverseLong( (void *)&longVal) ;
               // AS Jun 8/04 - for large key support, using a different trail byte counter
               return (int)( ( longVal >> pos ) & b4->dupByteCnt ) ;
            #else
               // AS Jun 8/04 - for large key support, using a different trail byte counter
               return (int)( ( *lPtr >> pos ) & b4->dupByteCnt ) ;
            #endif
         #endif
      }
      ...
      int S4FUNCTION x4trailCnt( const B4BLOCK *b4, const int numInBlock )
      {
         ...
         else
         {
            ...
            #endif
            // AS Jun 8/04 - for large key support, using a different trail byte counter
            return (int)( ( longPtr >> pos ) & b4->trailByteCnt ) ;
         #else
            #ifdef S4DO_BYTEORDER
               longVal = *lPtr ;
               longVal = x4reverseLong( (void *)&longVal) ;
               // AS Jun 8/04 - for large key support, using a different trail byte counter
               return (int)( ( longVal >> pos ) & b4->trailByteCnt ) ;
            #endif
            // AS Jun 8/04 - for large key support, using a different trail byte counter
            // return (int)( ( *lPtr >> pos ) & b4->nodeHdr.trailByteCnt ) ;
            return (int)( ( *lPtr >> pos ) & b4->trailByteCnt ) ;
         #endif
      }
  ...
  */
   ...
   int b4insertLeaf( B4BLOCK *b4, const void *vkeyData, const S4UNSIGNED_LONG rec )
   {
      ...
      */
      ...
      // AS Apr 16/10 - do a check that the keyLen is not too large (corrupt index)
      if ( keyLen > I4MAX_KEY_SIZE_COMPATIBLE && tag->codeBase->limitKeySize == 1 )
      {
         if ( tag->codeBase->oledbSchemaCreate == 1 )  // ensure via real max length (not-compatible)
         {
            if ( I4MAX_KEY_SIZE < keyLen )
      ...
      if ( b4numKeys( b4 ) == 0 )
      {
         ...
         #ifdef E4INDEX_VERIFY
            assert5( !b4leaf( b4 ) || ( b4->curPos >= (char *)(&b4->nodeHdr) + sizeof( b4->nodeHdr ) + b4->nodeHdr.freeSpace - reqdLen ) ) ;
         #endif
      ...
      else
      {
         if ( b4->keyOn == b4numKeys( b4 ) )  /* at end */
         {
            ...
            b4->curPos -= reqdLen ;
            #ifdef E4INDEX_VERIFY
               assert5( b4->curPos > (char *)(&b4->nodeHdr) + sizeof( b4->nodeHdr ) ) ;
            #endif
         ...
         else
         {
            ...
            #ifdef E4INDEX_VERIFY
               assert5( !b4leaf( b4 ) || ( b4->curPos > (char *)(&b4->nodeHdr) + sizeof( b4->nodeHdr ) ) ) ;
            #endif
   ...
   int b4doFlush( B4BLOCK *b4 )
   {
      ...
      b4getFilePosition( b4, &fPos ) ;
         ...
         u4free( swap ) ;
      #else
         // AS Apr 16/10 - ensure that fPos is ok
         if ( file4longGetHi( fPos ) == 0L )
         {
            if ( file4longGetLo( fPos ) < 1024 )  // we are about to write to an invalid position
               error4describe( b4->tag->codeBase, e4index, E90438, b4->tag->alias, 0, 0 ) ;
         }
   ...
   B4KEY_DATA * S4FUNCTION b4key( B4BLOCK *b4, const int iKey )
   {
      ...
      // AS Feb 3/04 - Fox 8.0 support, if the '2' bit is on, it is a leaf block, not if >= 2
      if ( b4->header.nodeAttribute & 0x02 ) /* leaf */
      {
   ...
   int b4dataLeaf( void *data, TAG4FILE *tag )
   {
      ...
      // AS Feb 3/04 - Fox 8.0 support, if the '2' bit is on, it is a leaf block, not if >= 2
      return( header->nodeAttribute & 0x02 ) ;
   }
      ...
      int b4leaf( const B4BLOCK *b4 )
      {
         ...
         return( b4->header.nodeAttribute & 0x02 ) ;
      }
   #endif /* S4INLINE */
   ...
   /* S4FOX */
      ...
      unsigned long S4FUNCTION b4recNo( const B4BLOCK *b4, const int i )
      {
         ...
         // AS Feb 3/04 - Fox 8.0 support, if the '2' bit is on, it is a leaf block, but a value
         // of '5' (bits 1&3) means it is not.
         if ( b4->header.nodeAttribute & 0x02 ) /* leaf */
            return x4recNo( b4, i ) ;
         else /* branch */
   ...
   int b4removeLeaf( B4BLOCK *b4 )
   {
      ...
      */
         ...
         {
            char dump[255];
            sprintf( dump, "b4removeLeaf called for Tag %s, block: %ld, keyOn =%ld, NumKeys = %ld \r\n", b4->tag->alias, (long)b4node(b4->fileBlock), (long)b4->keyOn, (long)b4numKeys(b4) ) ;
            log5( dump ) ;
         }
      #endif
   ...
   int b4remove( B4BLOCK *b4 )
   {
      ...
      if ( b4->header.nodeAttribute & 0x02 ) /* leaf */
         ...
         {
            char dump[255];
            sprintf( dump, "b4remove branch called for Tag: %s, block: %ld, keyOn =%ld, NumKeys = %ld \r\n", b4->tag->alias, (long)b4node(b4->fileBlock), (long)b4->keyOn, (long)b4numKeys(b4) ) ;
            log5( dump ) ;
         }
      #endif
   ...
   int S4FUNCTION b4skip( B4BLOCK *b4, int numToSkip )  // can only skip by int amount within block
   {
      ...
      // AS Feb 3/04 - Fox 8.0 support, if the '2' bit is off, it is a branch block
      if ( !(b4->header.nodeAttribute & 0x02) )  /* branch */
      {
         int numSkipped ;
         if ( numToSkip > 0 )
      ...
      void b4verifyPointers( const B4BLOCK *b4 )
      {
         ...
         for ( int blockPointersIndex = 0 ; blockPointersIndex < b4->tag->header.keysMax ; blockPointersIndex++ )
         {
            if ( b4->pointers[blockPointersIndex] >= lastPointer )
            {
               ...
               // assert5( b4->pointers[blockPointersIndex] < lastPointer ) ;  AS Jan 3/05 - already erroring out, don't assert...
               return ;
            }
   ...
   int S4FUNCTION b4skip( B4BLOCK *b4, int n )
   {
      ...
      int b4verify( B4BLOCK *b4 )
      {
         ...
         #ifdef S4FOX
            char *key = (char *)b4keyKey( b4, 0 ) ;
            // ensure the key fall into range
            assert5( !b4leaf( b4 ) || ( b4->builtPos > (char *)(&b4->nodeHdr) + sizeof( b4->nodeHdr ) + b4->nodeHdr.freeSpace ) ) ;
         #endif

         for ( i = 0 ; i < b4numKeys( b4 ) - 1 ; i++ )
         {
            #ifdef S4FOX
               assert5( !b4leaf( b4 ) || (b4->dupByteCnt != 0 && b4->trailByteCnt != 0) ) ;
               key = (char *)b4keyKey( b4, i+1 ) ;
               // ensure the key fall into range
               // assert5( !b4leaf( b4 ) || ( b4->builtPos > (char *)(&b4->nodeHdr) + sizeof( b4->nodeHdr ) + b4->nodeHdr.freeSpace ) ) ;
