/* c4com.h   (c)Copyright Sequiter Software Inc., 1988-1998.  All rights reserved. */
...
#else
   #define packet4len( p ) ( (long)ntohl((p)->dataLen) )
   #define packet4setLen( p, l ) ( ( (p)->dataLen = (htonl(l)) ), 0 )
   #define packet4setStatus( p, s ) ( ( (p)->status = (htons(s)) ) )
   #define packet4setType( p, t ) ( ( (p)->type = (htons(t)) ) )
   #define packet4status( p ) ( (short)(ntohs((p)->status )))
   #define packet4setErrCode2( p, e ) ( ((p)->errCode2 = (htonl(e))), 0 )
   #define packet4errCode2( p ) ( ntohl((p)->errCode2 ))
   #define packet4type( p ) ( ntohs((p)->type) )
   #define packet4serverId( p ) ( ntohl((p)->serverDataId ))
   #define packet4clientId( p ) ( ntohl((p)->clientDataId ))
   #define packet4readLock( p ) ( ntohs((p)->readLock) )
   #define packet4unlockAuto( p ) ( ntohs((p)->unlockAuto) )
   #define packet4setClientId( p, i ) ( ((p)->clientDataId = (htonl(i))) )
   #define packet4setServerId( p, i ) ( ((p)->serverDataId = (htonl(i))) )
   #define packet4setReadLock( p, r ) ( ((p)->readLock = (htons(r))) )
   #define packet4setUnlockAuto( p, v ) ( ((p)->unlockAuto = (htons(v))) )
