...
#include "d4all.h"
      ...
      if ( t4->indexFile->dataFile->compatibility == 30 )
      {
         if ( expr4nullLow( t4->expr, 0 ) )
         {
            ...
            *ptr = (char)0x80 ;
            ptr++ ;
            hasNull = 1 ;
            // AS Sept 19/13 - if the input len matches the keylen exactly we need to redue it by 1 (i.e. the case where the key to seek for is passing in extra characters)
   ...
   int S4FUNCTION d4seekDouble( DATA4 *data, const double dkey )
   {
      ...
      int S4FUNCTION d4seekLongLong( DATA4 *data, const LONGLONG dkey )
      {
         ...
         #else
            #ifndef S4FOX
               return error4( data->codeBase, e4notSupported, E92904 ) ;
            #else
               TAG4 *tag ;
               TAG4FILE *tfile ;
               CODE4 *c4 ;
