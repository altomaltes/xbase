...
/* LY 2001/08/21 : added ifdef to avoid compile error on non-WIN32 */
   ...
   #include <sys\stat.h>
      ...
      unsigned long data4lockId( DATA4 *d4 )
      {
         // in server, the lock id == client id, since for clones the client sends the lock id down directly in place
         // of the normal clientId
         return d4->clientId ;
      }
   #endif
...
void S4FUNCTION d4aliasSet( DATA4 *data, const char * newAlias )
{
   ...
   {
      // AS Nov 1/2013 - we should also update the index->accessName to match the new alias.
      // Just do fix in stand-alone for now, it isn't clear it is required in client/server,
      //and if so will be more complicated since will need to update indexname on server too.  Also i4setFileName only exists in stand/alone.
      #ifndef S4OFF_INDEX
         #ifdef S4STAND_ALONE
         #ifndef S4CLIPPER
            INDEX4 *index ;
            index = d4index( data, 0 ) ;  // get index matching alias if it exists
         #endif
         #endif
      #endif
      ...
      d4aliasFix( data ) ;
         ...
         #ifndef S4CLIPPER
            if ( index != 0 )
               i4setFileName( index, data, 0 ) ; // use the nex alias to reset the index accessName
         #endif
         #endif
      #endif
   ...
   */
   ...
   int d4readOld( DATA4 *data, const long recNum )
   {
      int rc ;
         ...
         {
            char dump[255] ;
            sprintf( dump, "d4readOld for innput recNo %ld, current recno: %ld oldRecord: %ld   record/old record:\r\n", recNum, (long)d4recNo(data), (long)data->recNumOld ) ;
            log5( dump );
            log5( data->record ) ;
            log5( "\r\n" );
            log5( data->recordOld ) ;
      ...
      data->recNumOld = recNum ;
         ...
         {
            char dump[255] ;
            sprintf( dump, "d4readOld COMPLETE for innput recNo %ld, current recno: %ld oldRecord: %ld   record/old record:\r\n", recNum, (long)d4recNo(data), (long)data->recNumOld ) ;
            log5( dump );
            log5( data->record ) ;
            log5( "\r\n" );
            log5( data->recordOld ) ;
...
// AS Mar 20/06 - make avail in server for modify function
   ...
   void S4FUNCTION d4recall( DATA4 *data )
   {
      ...
      if ( *data->record != ' ' )
      {
         // AS May 18/04 - already was not included in server case
         // AS Mar 20/06 - moved function back into server
         #ifndef S4SERVER
            #ifndef S4OFF_ENFORCE_LOCK
               if ( data->codeBase->lockEnforce && data->recNum > 0L )
   ...
   if ( data->dataFile->numRecs >= 0L )
   {
         ...
         if ( assumeLocked == !ASSUME4LOCKED )
         {
            int isLocked = d4lockTestAppend( data ) ;
               ...
               {
                  // Try with the clients client and server id to see if locked there
...
unsigned long S4FUNCTION d4recWidthLow( DATA4 *data )
{
      ...
      {
         // AS Jul 24/03 - also include 1 byte for the deleted flag
         data->exposedRecordLen = 1 ;

         // calculate it by adding up the field widths
         for ( unsigned short fieldIndex = 0 ; fieldIndex < data->dataFile->nFields ; fieldIndex++ )
         {
            assert5( (long)data->fields[fieldIndex].offset == (long)data->exposedRecordLen ) ;
...
// AS Jun 13/03 - expose for odbc, to support generic dll
#if !defined( S4SERVER )
